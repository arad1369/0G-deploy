...
const DeployHelloContract = ({ walletAddress }) => {
  ...
  const handleDeploy = async () => {
    try {
      ...
      setDeployStatus("Deploying the contract..."); const provider = new ethers.BrowserProvider(window.ethereum); const signer = await provider.getSigner(); const network = await provider.getNetwork(); if (Number(network.chainId) !== 16601) { setDeployStatus("(Chain ID: 16601)plaes connect 0g"); return; }
