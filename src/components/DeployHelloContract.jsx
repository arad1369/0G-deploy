...
const DeployHelloContract = ({ walletAddress }) => {
  ...
  const handleDeploy = async () => {
    try {
      if (!window.ethereum) { setDeployStatus("لطفاً MetaMask را نصب کنید."); return; }
  ...
  return (<div style={{ textAlign: "center", marginTop: "20px" }}>
    <button onClick={handleDeploy} style={{ padding: "10px 20px", fontSize: "16px", backgroundColor: "#00ff00", color: "#1a1a1a", border: "none", cursor: "pointer", margin: "10px", }}
    >
      Deploy Hello Contract
    </button>
    {deployStatus && (<p style={{ color: deployStatus.includes("خطا") ? "red" : "#00ff00", fontSize: "16px", marginTop: "10px" }}>
      {deployStatus}
    </p>)}
    {contractAddress && (<p style={{ color: "#00ff00", fontSize: "16px", marginTop: "10px" }}>address contract: {contractAddress}</p>)}
    {txHash && <p style={{ color: "#00ff00", fontSize: "16px", marginTop: "10px" }}>هش تراکنش: {txHash}</p>} </div>);
};
